/**
 * Bulk-reskill CDN loader.
 *
 * Loads the React/Redux UMD dependencies from a public CDN as browser globals,
 * then loads the widget bundle (bulk-reskill-cdn.js) which expects those globals.
 *
 * Why: when several widgets share the SAME CDN URLs, the browser downloads each
 * library ONCE and every other widget/loader gets a cache hit (and the same
 * in-memory global). A loader that already finds a global present skips it, so a
 * second widget only fetches its own (small) bundle.
 *
...
(function () {
  ...
  var WIDGET_URL = base + 'bulk-reskill-cdn.js';
  ...
  function loadScript(url, cb) {
    ...
    s.onerror = function () { console.error('[bulk-reskill loader] failed to load', url); cb(); };
