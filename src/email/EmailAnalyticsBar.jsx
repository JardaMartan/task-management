...
/**
 * EmailAnalyticsBar — interactive analytics strip above the email widget.
 * onFilterChange({ type: 'status'|'topic', key })
 */
const EmailAnalyticsBar = ({ darkMode, data: dataProp, onFilterChange, activeFilters = {}, onCaseClick }) => {
  ...
  const handleStatus = (key) => onFilterChange?.({ type: 'status', key });
  const handleTopic  = (key) => onFilterChange?.({ type: 'topic',  key });
  ...
  return (
    <div className={`analytics-bar analytics-bar--email${darkMode ? ' analytics-bar--dark' : ''}`}>
      ...
      <div className="analytics-section">
        <div className="analytics-section__title">{t('analytics.email.threads')}</div>
        <div className="analytics-section__content analytics-section__content--row">
          <div className="analytics-donut-wrap">
            <DonutChart segments={d.threadStatus} size={80} thickness={14} darkMode={darkMode}
              onSegmentClick={handleStatus} activeKey={activeFilters.status} />
          ...
          <Legend items={d.threadStatus} onItemClick={handleStatus} activeKey={activeFilters.status} />
        </div>
      </div>
      ...
      {/* ── Topic mix stack ── */}
      <div className="analytics-section">
        <div className="analytics-section__title">{t('analytics.email.topics')}</div>
        <div className="analytics-section__content">
          <StackBar segments={d.topicMix} height={8}
            onSegmentClick={handleTopic} activeKey={activeFilters.topic} />
          <Legend items={d.topicMix} onItemClick={handleTopic} activeKey={activeFilters.topic} />
        </div>
      </div>
      ...
      <div className="analytics-section analytics-section--sparklines">
        ...
        <div className="analytics-section__content">
          <div className="analytics-sparkline-row">
            ...
            <Sparkline values={d.volumeTrend.slice(-trendDays)} color="#a78bfa" width={84} height={28} />
          </div>
          <div className="analytics-sparkline-row">
            <span className="analytics-sparkline-label">{t('analytics.email.replyTimeH')}</span>
            <Sparkline values={d.replyTimeTrend.slice(-trendDays)} color="#f5a623" width={84} height={28} />
          </div>
        </div>
      </div>
      ...
      <div className="analytics-section analytics-section--kpis">
        ...
        <div className="analytics-section__content analytics-kpi-grid">
          <KpiTile label={t('analytics.email.kpi.avgFirstReply')} value={`${d.avgFirstReplyH}h`} accent="#00a0d1" />
          <KpiTile label={t('analytics.email.kpi.slaMet')}        value={`${d.slaMet}%`}        accent="#4ade80" />
          <KpiTile label={t('analytics.email.kpi.csat')}          value={d.csat.toFixed(1)} sub={t('analytics.cases.kpi.outOf5')} accent="#a78bfa" />
          <KpiTile label={t('analytics.email.kpi.threads')}       value={d.emailsThisMonth}  accent="#f5a623" />
        </div>
      </div>
