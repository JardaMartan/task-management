...
/**
 * ChatAnalyticsBar — interactive analytics strip for the chat widget.
 * onFilterChange({ type: 'channel'|'status', key })
 */
const ChatAnalyticsBar = ({ darkMode, data: dataProp, onFilterChange, activeFilters = {}, onCaseClick }) => {
  ...
  const handleChannel = (key) => onFilterChange?.({ type: 'channel', key });
    ...
    <div className={`analytics-bar analytics-bar--chat${darkMode ? ' analytics-bar--dark' : ''}`}>
      ...
      <div className="analytics-section">
        ...
        <div className="analytics-section__content analytics-section__content--row">
          <div className="analytics-donut-wrap">
            <DonutChart segments={d.channelMix} size={80} thickness={14} darkMode={darkMode}
              onSegmentClick={handleChannel} activeKey={activeFilters.channel} />
          ...
          <Legend items={d.channelMix} onItemClick={handleChannel} activeKey={activeFilters.channel} />
        </div>
      </div>
      ...
      <div className="analytics-section">
        ...
        <div className="analytics-section__content">
          ...
          <Legend items={d.sessionStatus} onItemClick={handleStatus} activeKey={activeFilters.status} />
        </div>
      </div>
      ...
      <div className="analytics-section analytics-section--sparklines">
        ...
        <div className="analytics-section__content">
          <div className="analytics-sparkline-row">
            ...
            <Sparkline values={d.volumeTrend.slice(-trendDays)} color="#25D366" width={84} height={28} />
          </div>
          <div className="analytics-sparkline-row">
            <span className="analytics-sparkline-label">{t('analytics.chat.ahtMin')}</span>
            <Sparkline values={d.ahtTrend.slice(-trendDays)} color="#f5a623" width={84} height={28} />
          </div>
        </div>
      </div>
      ...
      <div className="analytics-section analytics-section--kpis">
        ...
        <div className="analytics-section__content analytics-kpi-grid">
          <KpiTile label={t('analytics.chat.kpi.firstResponse')} value={`${d.avgFirstResponseSec}s`} accent="#25D366" />
          <KpiTile label={t('analytics.chat.kpi.slaMet')}        value={`${d.slaMet}%`}             accent="#4ade80" />
          <KpiTile label={t('analytics.chat.kpi.csat')}          value={d.csat.toFixed(1)} sub={t('analytics.cases.kpi.outOf5')} accent="#a78bfa" />
          <KpiTile label={t('analytics.chat.kpi.convos')}        value={d.conversationsThisMonth}   accent="#f5a623" />
        </div>
      </div>
