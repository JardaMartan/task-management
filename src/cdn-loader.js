/**
 * task-management CDN loader.
 *
 * Loads the React/Redux UMD dependencies from a public CDN as browser globals,
 * then loads the widget bundle (task-management-cdn.js) which expects them.
...
(function () {
  ...
  // React + ReactDOM. RTK stays BUNDLED in task-management (immer auto-freeze),
  // so it is not loaded here. Versions must match package.json (shared cache).
  var DEPS = [
  ...
  function loadScript(url, cb) {
    ...
    s.onerror = function () { console.error('[task-management loader] failed to load', url); cb(); };
