...
/**
 * Compact combo dropdown with an embedded search field. Momentum-token styled.
 * `options` = [{ id, name }]. `firstOption` = { id, label } is pinned first and
 * never filtered. The popover is fixed-positioned so it escapes scroll clipping
 * and works inside the shadow-root host (composedPath outside-click).
 */
export default function SearchableSelect({
  ...
  const filtered = q ? baseItems.filter((i) => i.label.toLowerCase().includes(q)) : baseItems;
  ...
  const allItems = firstOption
    ? [{ id: firstOption.id, label: firstOption.label }, ...baseItems]
    : baseItems;
  ...
  return (
    <div className="exp-ss" ref={rootRef} onKeyDown={onKeyDown}>
      <button
        type="button"
        ref={triggerRef}
        className={`exp-ss__trigger${open ? ' is-open' : ''}${selected ? '' : ' is-placeholder'}`}
      ...
      >
        <span className="exp-ss__value" title={triggerText}>{triggerText}</span>
        <span className="exp-ss__chevron"><ChevronIcon /></span>
      </button>
      {open && pos && (
        <div className="exp-ss__pop" style={{ position: 'fixed', top: pos.top, left: pos.left, minWidth: pos.width }}>
          <div className="exp-ss__search">
            <span className="exp-ss__search-icon"><SearchIcon /></span>
            <input
              ref={inputRef}
              className="exp-ss__input"
              type="text"
              value={query}
          ...
          <ul className="exp-ss__list" role="listbox" ref={listRef}>
            {items.map((it, i) => (
              <li
                key={it.id || '__first'}
                role="option"
                aria-selected={it.id === selectedId}
                className={`exp-ss__opt${it.id === selectedId ? ' is-selected' : ''}${i === activeIndex ? ' is-active' : ''}${it.pinned ? ' exp-ss__opt--pinned' : ''}`}
                onMouseEnter={() => setActiveIndex(i)}
                onMouseDown={(e) => { e.preventDefault(); pick(it.id); }}
                title={it.label}
              >
                <span className="exp-ss__opt-label">{it.label}</span>
                {it.id === selectedId && <span className="exp-ss__opt-check"><CheckIcon /></span>}
              </li>
            ))}
            {items.length === 0 && <li className="exp-ss__opt exp-ss__opt--empty">—</li>}
          </ul>
        </div>
      )}
    </div>
